...
4. Creatinine phosphokinase, platelets clearly has an outlier from max value which 
...
```
\
We notice some outliers at the positive side in 
creatinine_phosphokinase with data above median 
more dispersed \
...
creatinine_phosphokinase). However these are in possible ranges medically.\
...
seems to be higher but what about averages ? \
...
```
\
The distribution looks pretty normal with some tail noticeable
